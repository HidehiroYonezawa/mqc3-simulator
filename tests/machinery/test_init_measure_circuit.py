...
from mqc3.graph.ops import Initialization, Measurement
...
from tests.utility import check_mean_value, check_variance_value, initialized_squeezed_variances
...
def run_circuit(
    ...
    *,
    ...
    Args:
        ...
        theta (float): Initialization angle in radians.
        swap (bool): Swap parameter in the initialization macronode.
    ...
    graph_repr.place_operation(Initialization(macronode=(0, 0), theta=theta, initialized_modes=(0, 1)))
    ...
    pb_result_bytes = simulate_machinery(pb_machinery_bytes, "multi_cpu_peak", n_shots, squeezing_level)
...
def test_init_measure_circuit(theta: float) -> None:
    """Perform a initialization and measurement, then analyze the resulting data.

    This test executes an initialization macronode, followed by a measurement macronode,
    ...
    Args:
        theta (float): Initialization angle.
    """
    n_shots = 1000
    squeezing_level = 15
