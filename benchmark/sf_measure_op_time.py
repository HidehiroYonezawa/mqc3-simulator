...
def measure_op_time_measurement(n_modes: int, n_cats: int) -> None:
    mode_idxs = list(range(n_modes))
    ...
    with prog.context as q:
        ...
        for mode in mode_idxs:
            ...
            BSgate(theta=theta, phi=phi) | (q[mode], q[mode + 1])
        random.shuffle(mode_idxs)
        for mode in mode_idxs:
            MeasureHomodyne(phi=secrets.SystemRandom().uniform(-pi, pi)) | q[mode]
...
for n_modes in v_n_modes:
    for n_cats in v_n_cats:
        measure_op_time_measurement(n_modes=n_modes, n_cats=n_cats)
