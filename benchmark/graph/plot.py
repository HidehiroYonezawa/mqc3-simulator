...
def calc_predicted_circuit_simulation_ms(
        n_local_macronodes: int, n_steps: int, rotation_op_ms: float, measurement_op_ms: float) -> float:
    n_rotation_ops = n_local_macronodes * (n_steps - 2) * 2
    n_measurement_ops = n_local_macronodes * 2 * 2
    return rotation_op_ms * n_rotation_ops + measurement_op_ms * n_measurement_ops


def plot_predicted_circuit_simulation_time(
        ax: axes.Axes, n_local_macronodes: int, all_n_steps: list[int],
        rotation_op_ms: float, measurement_op_ms: float) -> None:
    predicted = [
        calc_predicted_circuit_simulation_ms(n_local_macronodes, n_steps, rotation_op_ms, measurement_op_ms)
...
if __name__ == "__main__":
    ...
    columns = ["InitializeGraphModes", "ConvertGraphToCircuit", "SimulateCPUMultiThread", "ExtractGraphResult"]

    plot(ax1, data, ("n_local_macronodes", 101), "n_steps", columns)
    plot(ax2, data, ("n_local_macronodes", 500), "n_steps", columns)

    all_n_steps = sorted(data["n_steps"].unique())
    plot_predicted_circuit_simulation_time(ax=ax1, n_local_macronodes=101, all_n_steps=all_n_steps,
                                           rotation_op_ms=0.00279, measurement_op_ms=0.042075)
    plot_predicted_circuit_simulation_time(ax=ax2, n_local_macronodes=500, all_n_steps=all_n_steps,
                                           rotation_op_ms=0.01083, measurement_op_ms=0.939988)
