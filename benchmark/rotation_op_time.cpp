...
auto RotationOpTimeMeasurementConfig(const std::size_t n_modes, const std::size_t n_cats)
    ...
    // Circuit
    constexpr std::size_t NRepeat = 10;
    auto mode_idxs = std::vector<std::size_t>{};
    mode_idxs.reserve(n_modes * NRepeat);
    for (std::size_t mode = 0; mode < n_modes; ++mode) {
        for (std::size_t j = 0; j < NRepeat; ++j) { mode_idxs.push_back(mode); }
    }
    std::shuffle(mode_idxs.begin(), mode_idxs.end(), gen);
    auto circuit = bosim::Circuit<double>{};
    for (const auto mode : mode_idxs) {
        circuit.EmplaceOperation<bosim::operation::PhaseRotation<double>>(mode, angle_dist(gen));
    }
...
int main(int argc, char** argv) {
    ...
    auto [circuit, state] = RotationOpTimeMeasurementConfig(n_modes, n_cats);
